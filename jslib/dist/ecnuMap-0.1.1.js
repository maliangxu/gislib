...
var Canvas2dRenderer = (function Canvas2dRendererClosure() {
  ...
  Canvas2dRenderer.prototype = {
    ...
    _setStyles: function(config) {
      ...
      this.canvas.style.zIndex = config.zIndex || 10;
    },
...
gEcnu.Geocoder = gClass.extend({
    init: function(eventsListener) {
        ...
        this.fields.lat = null;   
        this.fields.lon = null;   
        this.fields.x = null;   
        this.fields.y = null;    
        this.city = null;
    },
    geocoder:function(options){
        ...
        this.tbname = options.tb;
        this.fields.address = options.fields.address || 'address';      
        this.fields.lat = options.fields.lat || 'lat';   
        this.fields.lon = options.fields.lon || 'lon';   
        this.fields.x = options.fields.x || 'x';   
        this.fields.y = options.fields.y || 'y';    
        this.city = options.city || '上海市';
        var script = document.createElement('script');
        script.src = 'http://api.map.baidu.com/api?v=2.0&ak=wDoIGnshwU8LjXz444pkYvz8&callback=BMapGeocoder';
        document.getElementsByTagName('head')[0].appendChild(script);
        
        var that = this;
        function BMapGeocoder(){
            var myGeo = new BMap.Geocoder();
            that.getAddress(function(adds){
                that.geocodeSearch(adds,0,myGeo);
            });
        };
    },
    getAddress: function (succ) { 
        var sql = {'lyr':this.tbname, 'fields':this.fields.address, 'filter':''};
        var sqlservice=new gEcnu.WebSQLServices.SQLServices({'processCompleted':function(tmpmsg){
                var adds = [];
                var len=tmpmsg.length;
                for(var i=0;i<len;i++){
                    var address=tmpmsg[i]['address'];
                    adds.push(address);
                }
                succ(adds); //回调函数里返回数据
            },'processFailed':function(){}});
    ...
    geocodeSearch: function (adds,index,myGeo){
        var that = this;
        if(index < adds.length){
            setTimeout(function(){
                index++;
                that.geocodeSearch(adds,index,myGeo);
            }, 20);
        } 
        var add = adds[index];
        myGeo.getPoint(add, function(point){
            if (point) {
                that.BD2WGSOnce(point.lat,point.lng,add);
            }
        }, this.city);
    },
    BD2WGSOnce: function (lat,lon,add) {
        var that = this;
        var url="http://api.zdoz.net/bd2wgs.aspx?lat="+lat+"&lng="+lon;
        $.ajax({
            url:url,
            type: 'GET',
            success:function(tmpmsg){ 
                var wxy_cur = that.lnglatToSh2(tmpmsg['Lng'], tmpmsg['Lat']);//上海市坐标
                var tmp={address:add,lat:tmpmsg['Lat'],lon:tmpmsg['Lng'],x:wxy_cur.x,y:wxy_cur.y};
                that.updateAddress(tmp);
            },
            dataType:"jsonp",
            async:false
        }); 
    },
    lnglatToSh2: function (lng,lat) {
        var A = 95418.0172735741;
        var B = 48.3052839794785;
        var C = -11592069.1853624;
        var D = 63.9932503167748;
    ...
    updateAddress: function (tmp){
        var that = this;
        var fields = [this.fields.address, this.fields.lat, this.fields.lon, this.fields.x, this.fields.y];
        var data = [tmp.address, tmp.lat, tmp.lon, tmp.x, tmp.y];
        var params = {'Fields':fields, 'Data':[data]};
        var sqlservice=new gEcnu.WebSQLServices.SQLServices({'processCompleted':function(msg){
            if (typeof(that.events._events.processCompleted) != "undefined") {
                var jsonparase=JSON.parse(msg);
                that.events._events.processCompleted(jsonparase);
            }
        },'processFailed':function(){}});
